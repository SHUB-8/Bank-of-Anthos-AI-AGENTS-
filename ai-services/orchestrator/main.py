...
def create_gemini_tools():
    ...
    return Tool(function_declarations=[
        get_contacts_tool, add_contact_tool, resolve_contact_tool,
...
async def execute_tool_call(tool_call, claims: Dict[str, Any], auth_header: str):
    ...
    try:
        ...
        # Transaction Tools
            ...
            # If suspicious or fraud, return the result for user confirmation
            if anomaly_result["status"] in ["suspicious", "fraud"]:
                return {
                    "status": "requires_confirmation",
                    "risk_assessment": anomaly_result,
                    "message": f"This transaction has been flagged as {anomaly_result['status']}. Reasons: {', '.join(anomaly_result['reasons'])}"
                }
            ...
            transaction_result = await sage_services.execute_transaction(
                {
                    ...
                    "fromRoutingNum": args.get("routing_num", "883745000"),

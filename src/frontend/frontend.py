...
def create_app():
    ...
    def chat():
        ...
        try:
            ...
            app.logger.info(f"Conversational agent response status: {response.status_code}")
            ...
            if response.status_code == 200:
                try:
                    ...
                    return jsonify({"error": "Chat service returned invalid JSON"}), 503
            else:
                app.logger.error("Conversational agent error: %s %s", response.status_code, response.text)
            ...
            app.logger.error("Chat handler error details: %s", str(e))
            import traceback
